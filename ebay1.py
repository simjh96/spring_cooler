schedule = [["MO 12:00 WE 14:30", "MO 12:00", "MO 15:00", "MO 18:00"], ["TU 09:00", "TU 10:00", "TU 15:00", "TU 18:00"],
...
def get_range(st):
    # parse str to range -> [[,],[,]]
    if len(st) == 8:
        t_s = day_dict[st[:2]] * 10000 + int(st[3:5]) * 100 + int(st[6:]) * 10 // 6
        t_e = t_s + 300
        return [[t_s, t_e]]
    else:
        t1 = get_range(st[:8])[0]
        t2 = get_range(st[9:])[0]
        return [[t1[0], t1[1] - 150], [t2[0], t2[1] - 150]]


def _comp_schedule(my_s, cand_s):
    # is compatible? -> bool
    if (my_s[0] < cand_s[0] < my_s[1]) | (my_s[0] < cand_s[1] < my_s[1]):
        return False
    else:
        return True


def comp_schedule(my_ss, cand_ss):
    results = []
    for cs in cand_ss:
        results.append(all(map(lambda ms: _comp_schedule(ms, cs), my_ss)))
    #
    # print(f'my_ss: {my_ss}, results: {results}')
    return all(results)
...
def dfs(my_ss, i=0):
    if i == 5:
        ...
        for s in schedule[i]:
            cand_ss = get_range(s)
            if comp_schedule(my_ss, cand_ss):
                dfs(my_ss+cand_ss, i+1)
...
# [print(get_range(s)) for s in schedule[0]]
